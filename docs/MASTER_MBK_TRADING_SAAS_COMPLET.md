...
| 2 | Data feed CME (GC,HG,CL,ES,6J,MBT) | Rithmic via NTB | ~50$ |
| 3 | Exchange fee CBOE (VIX) | CBOE | ~10$ |
| 4 | ATAS Pro (order flow complet) | atas.net | ~70$ |
| 5 | Claude API (Sonnet — cerveau KB) | Anthropic | ~15-20$ |
...
GÉOPOLITIQUE_IMPACT = {
    "Moyen-Orient_tension": {"GC": +1, "CL": +1, "ES": -1, "6J": +0.5},
    "USA_Chine_tension":    {"HG": -1, "6J": +1, "BTC": 0},
    "Crise_bancaire":       {"GC": +2, "6J": +2, "ES": -2},
...
```python
import pandas as pd
# Calculé sur les données NT8 historiques
CORRÉLATIONS_CLÉS = {
    "GC_DX":    "inverse  (ρ ~ -0.85)",
    "ES_VX":    "inverse  (ρ ~ -0.90)",
    "BTC_ES":   "positive (ρ ~ +0.65)",
    "CL_DX":    "inverse  (ρ ~ -0.70)",
    "HG_ES":    "positive (ρ ~ +0.70)",
    "6J_VX":    "positive (ρ ~ +0.55)",
}
```

### 7B — Détecteur de régime de marché
```python
def detect_regime(vix, sp500_slope, gold_slope, dollar_slope):
    if vix < 15 and sp500_slope > 0:
        return "RISK_ON"      # Favoriser ES, CL, HG, BTC
    elif vix > 20 and gold_slope > 0:
        return "RISK_OFF"     # Favoriser GC, 6J / éviter ES
    elif gold_slope > 0 and oil_slope > 0:
        return "INFLATION"    # Favoriser GC, CL / éviter 6J
    elif vix > 35:
...
# SECTION 10 — QUESTIONS IMMÉDIATES AVANT DE DÉMARRER

**Réponds à ces 3 questions — elles déterminent la semaine 1 :**

**Q1 : As-tu déjà une licence NinjaTrader 8 active (lifetime ou abonnement) ?**
→ Si OUI avec broker NTB : data feed CME inclus → économise ~50$/mois
→ Si NON : ouvrir compte broker NTB sur ninjatrader.com (gratuit)

**Q2 : As-tu Python 3.11 installé sur ton PC Windows ?**
→ Si NON : c'est la première chose à faire (15 minutes)

**Q3 : As-tu les 2337 règles de ta KB sous format texte quelque part ?**
→ C'est la pièce maîtresse du système — sans elles, Claude n'a pas de cerveau
→ Si elles sont dans ta tête ou dans des notes : il faut les formaliser avant la semaine 6

---

*DOCUMENT MASTER v1.0 — MBK Trading SaaS God Mode*
*Fusion de : Plan MBK v2.0 + OODA God Mode + GetXAPI + Stratégie 7 Cercles*
*Sources vérifiées | Budget ~150$/mois | 10 semaines de déploiement*
